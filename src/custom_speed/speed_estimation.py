...
class SpeedEstimator:
    ...
    def __init__(self, names, reg_pts=None, view_img=False, line_thickness=2, region_thickness=5, spdl_dist_thresh=10):
        ...
        # DataFrame
        self.df_speed = pd.DataFrame(columns=['track_id', 'speed'])
    ...
    def plot_box_and_track(self, track_id, box, cls, track):
        ...
        current_speed = int(self.dist_data[track_id]) if track_id in self.dist_data else 0
        ...
        # Dibujar la etiqueta y el cuadro del objeto
        self.annotator.box_label(box, speed_label, bbox_color,txt_color=(0, 0, 0))
        cv2.polylines(self.im0, [self.trk_pts], isClosed=False, color=(0, 255, 0), thickness=1)
        cv2.circle(self.im0, (int(track[-1][0]), int(track[-1][1])), 5, bbox_color, -1)
        return track_id, current_speed
    ...
    def estimate_speed(self, im0, tracks, region_color=(255, 0, 0)):
        ...
        speed_json = {"track_id": [], "speed": []}
        for box, trk_id, cls in zip(self.boxes, self.trk_ids, self.clss):
            track = self.store_track_info(trk_id, box)

            if trk_id not in self.trk_previous_times:
                self.trk_previous_times[trk_id] = 0
            track_id, current_speed = self.plot_box_and_track(trk_id, box, cls, track)
            if current_speed > 0:
                speed_json["track_id"].append(track_id)
                speed_json["speed"].append(current_speed)
                
            self.calculate_speed(trk_id, track)
        self.df_speed = pd.DataFrame(speed_json)
        if self.view_img and self.env_check:
            self.display_frames()
